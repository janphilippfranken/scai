...
class BuyerAgent(BaseAgent):
    ...
    def _get_chat_history_prompt_templates(
        ...
        if chat_memory.get(f"{self.model_id}_buyer") is None or len(chat_memory[f"{self.model_id}_buyer"]) == 0: # if we are at the beginning of a conversation
            chat_history_prompt_templates = [
                HumanMessagePromptTemplate.from_template(
                    f"(user {self.model_id}) {task_prompt.preamble} '{task_prompt.content}' {task_prompt.buyer_connective}"
                )
            ]
        ...
        chat_history_prompt_templates = [
                template
                for buyer, user in zip(chat_memory[f"{self.model_id}_buyer"], chat_memory[f"{self.model_id}_user"])
        ...
        chat_history_prompt_templates.insert(0, HumanMessagePromptTemplate.from_template(f"(user {self.model_id}) {task_prompt.preamble} '{task_prompt.content}' {task_prompt.buyer_connective}")) # insert task prompt at the beginning
