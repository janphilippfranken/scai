...
META_PROMPTS: Dict[str, MetaPrompt] = {
     "meta_prompt_1": MetaPrompt(
        ...
        metrics=[],
        content="""
In this simulation, the majority of "fixed-policy agents" share some principles, which tells them to adhere to somes principles in their interactions while playing the dictator game. "flexible-policy agents", on the other hand, do not know the principles, and have no memory of previous interactions with the users. Your job is to infer this principle by analyzing user interactions, and then create a principle for "Flexible-policy agents" to follow so that "Flexible-policy agents" behaves EXACTLY the same as the majority of the "fixed-policy agents". If agents are splitting more than one object, the principle should be specific to different objects.

Analyze the model fixed interactions, found below, that shows the behavior of all "fixed-policy agents" to infer the principle that the majority of agents are acting upon. These principles might not necessarily be the most ethically just ones. 
{fixed_string}
---------------------------
...
Now, if the flexible-policy agents behave differently than the majority of the fixed agents, revise the previous principle if necessary so that flexible-policy agent behaves EXACTLY like the majority of fixed-policy agents - don't be afraid to push the flexible-policy agent TO THE EXTREME, and keep this revised principle short. Remember, if flexible-policy agents' responses are exactly like the majority of the fixed-policy agents' responses, then the principle is correct and you shouldn't revise.
