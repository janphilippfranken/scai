...
class UserModel():
    ...
    def run(
        ...
        if test_run:
            ...
            print(prompt)
            print()
            return {'prompt': prompt, 'satisfaction': np.random.randint(1,4),'helpfulness': np.random.randint(5,7), 'harmlessness': np.random.randint(11), 'response': 'User_feedback_' + str(self.conversation_id)}
